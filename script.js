document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add a new global variable for tracking in-progress tasks and started tasks
...
// Create a dropdown for task status
const statusDropdown = document.createElement('select');
statusDropdown.classList.add('status-dropdown');
statusDropdown.innerHTML = `
    <option value="select">Select</option>
    <option value="To-Do">Started</option>
    <option value="In Progress">In Progress</option>
    
`;
statusDropdown.addEventListener('change', function () {
    updateInProgressTasks(); // Update in-progress tasks count
    updateTodoTasks(); // Update to-do tasks count
});

// Append status dropdown to task container
const statusContainer = document.createElement('div');
statusContainer.classList.add('status-container');
statusContainer.appendChild(statusDropdown);

// Append status container to task container
taskContainer.appendChild(statusContainer);
    ...
    // Function to update in-progress tasks count
function updateInProgressTasks() {
    const inProgressTaskDropdowns = document.querySelectorAll('.status-dropdown');
    let inProgressCount = 0;

    inProgressTaskDropdowns.forEach(dropdown => {
    ...
    // Update in-progress count in UI
    document.querySelector('.InProgress .count').textContent = inProgressTasks;
}
// Function to update to-do tasks count
function updateTodoTasks() {
    const todoTaskDropdowns = document.querySelectorAll('.status-dropdown');
    let todoCount = 0;

    todoTaskDropdowns.forEach(dropdown => {
        if (dropdown.value === 'To-Do') {
            todoCount++;
        }
    });
    ...
    // Update to-do count in UI
    //document.querySelector('.To-Do .count').textContent = todoTasks;
    // Update to-do count in the "Started" section of the footer
    document.querySelector('.Started .started-count').textContent = todoTasks;
}
// Update to-do tasks count after creating a new task
updateTodoTasks();
